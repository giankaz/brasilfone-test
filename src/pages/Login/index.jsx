...
export default function Login() {
  ...
  const schema = yup.object().shape({
		...
		email: yup
			.string()
			.required("Digite seu Email ou Celular!"),
		pass: yup
			.string()
    ...
    <StyledMain>
        ...
        <section className='form__container'>
            ...
            <div className='form__innerBox'>
              ...
              <InputBase label='E-mail ou celular' register={register} info='email' className={errors.email?.message && 'error'} errorMsg={''}/>
              <InputPass label='Senha' register={register} info='pass' className={errors.pass?.message && 'error'}  errorMsg={''}/>
              {loginError && 
              <StyledErrorContainer display={'flex'}>
                ...
                <span>{errors.email?.message ? errors.email.message : (errors.pass?.message ? errors.pass.message : 'Usuário ou senha incorretos, tente novamente.')}</span>
                </div>
              </StyledErrorContainer>
              }
    
              <Button width={'70%'} type='submit'>Conectar</Button>
              </div>
